...
class OrderLine {
	...
	public OrderLine(Item item, int quantity) throws Exception {
		...
		//This keyword to denote this class variable
		this.item = item;
...
class calculator {
	...
	public static double rounding(double value) {
		...
		// the integer value reduces the decimal digits of the cost
	...
	public void calculate(Map<String, Order> o) {
		...
		for (Map.Entry<String, Order> entry : o.entrySet()) {
			...
			// Should not be intialized in loop
			//grandtotal = 0;
			...
			for (int i = 0; i < r.size(); i++) {
				...
				// Remove math.floor and rounding should be done after adding
				double totalprice = rounding(rounding(r.get(i).getItem().getPrice()) + rounding(tax));
			...
			// Remove math floor and rounding should be done
			System.out.println("Sales Tax: " + rounding(totalTax));
			
			
			// Tax should be added along with grandtotal tax variable
			...
			grandtotal += total;
			grandtotalWithTax +=total;
            grandtotalWithTax +=totalTax;
...
public class Foo {
	...
	public static void main(String[] args) throws Exception {
		...
		//c.clear(); Bug: clears values in order
